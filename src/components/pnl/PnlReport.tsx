...
import { useTradeStore, Trade, BROKERAGE_PER_LEG, tradeNetPnl } from '@/store/tradeStore';
...
function SummaryCard({ label, trades }: { label: string; trades: Trade[] }) {
  ...
  const brokerage  = trades.reduce((s, t) => s + (t.status === 'closed' ? 200 : 100), 0);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Brokerage ₹{BROKERAGE_PER_LEG} per leg (entry &amp; exit each)</p>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          {filteredTrades.length > 0 && (() => {
            ...
            const brokerage = filteredTrades.reduce((s, t) => s + (t.status === 'closed' ? 200 : 100), 0);
            const net       = closed.reduce((s, t) => s + tradeNetPnl(t), 0);
            return (
              <div className="flex items-center gap-4 text-sm">
                <span className="text-muted-foreground">Gross <span className={cn('font-mono font-semibold', gross >= 0 ? 'text-profit' : 'text-loss')}>{fmt(gross)}</span></span>
                <span className="text-muted-foreground">Brok <span className="font-mono font-semibold text-loss">−₹{brokerage}</span></span>
                <span className="text-muted-foreground">Net <span className={cn('font-mono font-bold', net >= 0 ? 'text-profit' : 'text-loss')}>{fmt(net)}</span></span>
              </div>
            );
          })()}
        </div>
                ...
                {filteredTrades.map((trade) => {
                  ...
                  return (
                    <TableRow key={trade.id}>
                      ...
                      <TableCell className="text-right font-mono text-sm text-loss">
                        −₹{brok}
                      </TableCell>
