...
import { useWishlistStore, calcRFactor, WatchlistItem } from '@/store/wishlistStore';
...
function RBadge({ r }: { r: number | null }) {
  ...
  return <span className={cn('font-mono font-bold text-sm', color)}>{r}R</span>;
}
...
export function WatchlistTable() {
    ...
    return (
      <div className="text-center py-20 text-muted-foreground">
        ...
        <p className="text-xs mt-1">
          Search above or go to NSE Heatmap → click a sector → tap <strong>+</strong> next to any stock.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
                ...
                return (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-center">
                      <RBadge r={r} />
                      {r !== null && r >= 2 && <div className="text-[9px] text-profit mt-0.5">Good setup</div>}
                      {r !== null && r <  1 && <div className="text-[9px] text-loss  mt-0.5">Poor R</div>}
                    </TableCell>
      ...
      {/* Paper trade form pre-filled from watchlist row */}
      {tradeItem && (
        <PaperTradeForm
          open={!!tradeItem}
          onClose={() => setTradeItem(null)}
          prefill={{ symbol: tradeItem.symbol, underlyingType: 'equity' }}
        />
      )}
    </>
  );
