...
interface Stock {
  symbol: string; lastPrice: number; high: number; low: number;
  change: number; pctChange: number; volume: number; vwap: number;
  lastUpdated: string; series: string;
}
...
export function SectorStocks({ indexName, heatmapType = 'sectoral' }: { indexName: string; heatmapType?: HeatmapType }) {
  ...
  const sorted = [...stocks].sort((a, b) => {
    const va = Number(a[sort.key] ?? 0), vb = Number(b[sort.key] ?? 0);
    return sort.dir === 'asc' ? va - vb : vb - va;
  });

  const SH = ({ col, label, className }: { col: SortKey; label: string; className?: string }) => (
    <TableHead className={cn('cursor-pointer select-none hover:text-foreground whitespace-nowrap', className)}
            ...
            return (
              <TableRow key={s.symbol}>
                <TableCell className="pl-5">
                  ...
                  {s.series && s.series !== 'EQ' && <div className="text-[10px] text-muted-foreground">{s.series}</div>}
                </TableCell>
